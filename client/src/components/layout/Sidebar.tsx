...
export function Sidebar() {
  ...
  return <>
    <aside className="hidden w-64 shrink-0 bg-[var(--royal-blue)] text-white md:block"><div className="flex h-20 items-center gap-3 border-b border-[#28436c] px-6"><RRLogo size="medium" ariaLabel="RRVMS" /><div className="display text-lg font-bold">Visitor Management</div></div><div className="px-0 py-7">{(['Workspace', 'Governance', 'Account'] as const).map((group) => grouped[group] ? <div key={group}><p className="px-5 pb-3 pt-2 text-[10px] font-semibold uppercase tracking-[0.22em] text-[#8ea5c7]">{group}</p><LinkGroup items={grouped[group]} /></div> : null)}</div><div className="mt-auto border-t border-[#28436c] px-5 py-5 text-xs text-[#bfcde0]">RRVMS prototype<br /><span className="text-[#8098bc]">v0.1 foundation</span></div></aside>
    <details className="mobile-nav md:hidden"><summary aria-label="Open navigation">Menu <span aria-hidden="true">+</span></summary><div className="mobile-nav-panel">{(['Workspace', 'Governance', 'Account'] as const).map((group) => grouped[group] ? <div key={group}><p className="mobile-nav-label">{group}</p><LinkGroup items={grouped[group]} /></div> : null)}</div></details>
  </>
}
