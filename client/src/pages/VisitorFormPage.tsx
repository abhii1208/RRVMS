...
export function VisitorFormPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setSaving(true)
    try {
      const updatedRequest = formInfo?.status === 'SUBMITTED' || formInfo?.status === 'PENDING_DOCUMENTATION'
        ? await submitAdditionalVisitorForm(id, form)
        : await submitVisitorForm(id, form)
  ...
  const isAlreadySubmitted = formInfo?.status === 'SUBMITTED'
  const isRevisionAllowed = isAlreadySubmitted || formInfo?.status === 'PENDING_DOCUMENTATION'
  ...
  return (
    <form onSubmit={submit} className="mx-auto max-w-4xl space-y-6">
      ...
      <section className="border border-[var(--silver)] bg-white p-6">
        ...
        <button type="button" className="mt-4 cursor-pointer text-sm font-semibold text-[var(--royal-blue)]" onClick={addAsset}>+ Add asset</button>
      </section>
